...
Start with the product URL, who it serves, the problem it solves, its meaningful difference and the offer. Inspect the actual product where access is available. Separate verified product behavior from positioning ideas and unresolved assumptions.

Collect usable screenshots, app recordings, product photos, packaging, demonstrations and genuine customer language. Record the sources for claims and quotes. These assets establish what the ad can credibly show; do not invent testimonials or imply generated illustrative footage is a real customer demonstration.
...
- Direct competitors: how they explain the category and demonstrate the product.
...
- The hook and a brief shot/structure breakdown.
...
Turn the research into roughly three meaningful directions. Each should specify the audience problem, main message, hook, short script or script outline, product evidence to show, and the reference elements informing the filming and edit.
...
- Exact joins, cut timing and crop changes: FFmpeg or a video editor.

These are scoped starting choices, not universal model rankings. The approved Seedance food montage was not a controlled comparison against Gemini using the identical prompt.

Generate coverage that fits the intended edit. Fast montages often use short inserts; demonstrations and continuous reveals may need more time. A three-second source can supply enough coverage without generating four seconds automatically. Specify native shot changes when requesting a generated montage, and keep manual assembly instructions separate.

## 6. Assemble, review and make focused variants

Cut supporting footage against the actual spoken delivery. Review the whole export for clear meaning, accurate product details, believable action, pacing, continuity, complete dialogue and clean audio joins. Inspect dimensions and duration; use playback to judge motion and audio rather than relying solely on contact sheets.

Get one complete version working, then vary the hook, opening visual, demonstration or another deliberate creative choice while reusing good assets. Repair the smallest weak passage rather than regenerating everything. Preserve selected versions so feedback can refer to a concrete comparison.

Distinguish creative review from campaign performance. A visually approved ad is ready for its next intended use; it has not automatically proved conversion performance. Launching campaigns or publishing remains within the user's actual authorization.
