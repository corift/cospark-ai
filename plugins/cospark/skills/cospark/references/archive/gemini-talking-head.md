# Archived: Gemini Omni timed dialogue and action guide

Preserved for reference. This is not active guidance or an automatic fallback for product handling. Read it only when the user explicitly asks for the former timed Dialogue + Action approach. The active guide is [Generate realistic UGC clips](../gemini-talking-head.md).
